...
import { BarChart3, Download, TrendingDown, TrendingUp, Leaf, DollarSign, Users, AlertTriangle } from "lucide-react";
import { useToast } from "@/components/ToastProvider";

const stats = [
  { label: "Total Waste This Week", value: "48 kg", change: "-12%", up: false, icon: TrendingDown, color: "emerald" },
  { label: "Est. Cost Saved", value: "₹5,760", change: "+8%", up: true, icon: DollarSign, color: "blue" },
...
const wasteTrend = [
  { day: "Mon", kg: 12 }, { day: "Tue", kg: 8 }, { day: "Wed", kg: 15 },
  { day: "Thu", kg: 6 }, { day: "Fri", kg: 10 }, { day: "Sat", kg: 4 }
];
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-8">
        ...
        <div className="relative z-10 flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center"><BarChart3 size={22} /></div>
              <h1 className="text-2xl font-black">Staff Reports</h1>
            </div>
            <p className="text-blue-100 font-medium text-sm max-w-lg">Weekly analytics, waste trends, and operational alerts for canteen staff.</p>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
            ...
            <div className={`w-10 h-10 rounded-xl bg-${s.color}-500/10 text-${s.color}-500 flex items-center justify-center mb-4`}>
              <s.icon size={20} />
            </div>
            ...
            <div className={`flex items-center gap-1 mt-2 text-xs font-black ${s.up ? "text-emerald-500" : "text-emerald-500"}`}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div className="flex items-end gap-4 h-40">
          {wasteTrend.map((d, i) => (
            <motion.div key={d.day} initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} transition={{ delay: i * 0.1, duration: 0.4 }}
              style={{ originY: 1 }} className="flex flex-col items-center gap-2 flex-1">
              <span className="text-xs font-black text-slate-600 dark:text-slate-400">{d.kg}kg</span>
              <div className="w-full rounded-t-xl bg-gradient-to-t from-blue-600 to-blue-400 shadow-lg shadow-blue-500/20"
