...
export default function WasteAnalytics() {
  ...
  const avgPerDay = logs.length ? (logs.reduce((a, w) => a + (w.weight || 0), 0) / logs.length).toFixed(1) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Logs", value: logs.length, icon: "📋", color: "text-slate-700 dark:text-slate-200" },
          { label: "This Week (kg)", value: totalThisWeek.toFixed(1), icon: "📅", color: "text-rose-600" },
          { label: "Avg per Log (kg)", value: avgPerDay, icon: "⚖️", color: "text-amber-600" },
        ].map(s => (
          <div key={s.label} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 text-center shadow-sm">
