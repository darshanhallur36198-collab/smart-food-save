...
async function seedDatabase() {
  try {
    ...
    if (!studentExists) {
      ...
      await User.create({ name: "Demo Student", email: "student@foodsave.com", password: sp, role: "student" });
    ...
    if (!staffExists) {
      ...
      await User.create({ name: "Demo Staff", email: "staff@foodsave.com", password: stp, role: "staff" });
    ...
    console.log("✅ Seeded inventory (includes items expiring tomorrow to trigger Smart AI Suggestions & Low Stock alerts)");

    // 3. Add sample donations for Kanban Board
    await Donation.create([
      { foodAmount: 15, location: "Main Campus Cafeteria", status: "Pending" },
      { foodAmount: 22, location: "North Wing Dining", status: "Assigned", ngoName: "City Food Bank" },
      { foodAmount: 10, location: "East Hostel Mess", status: "Picked Up", ngoName: "Hope Shelter" },
      { foodAmount: 30, location: "Main Campus Cafeteria", status: "Delivered", ngoName: "Community Kitchen" }
    ]);
